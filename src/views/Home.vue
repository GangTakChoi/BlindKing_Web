<template>
  <div class="content-container">
    <div
      v-if="!$global.isLogin && !$global.isLoadingUserInfo"
      class="greeting-panel shadow"
    >
      ...
      <pre class="greeting-content">
        ...
        관리자 계정 => ID: admin / PW: 123123
        일반 계정 => ID: choikt / PW: 123123
    ...
    <div>
      <button
        @click="activeMatching"
        :disabled="isLoadActiveMatching || !$global.isLogin"
        class="matching-active-button basic-button-design shadow-sm rounded"
      >
        매칭 활성화
        <span
          class="on-off-button"
          :class="{
            'matching-on-status-style': isMatchingOnStyle,
            'matching-off-status-style': isMatchingOffStyle,
          }"
        >
          <div
            v-if="isLoadActiveMatching"
      ...
      <button
        v-if="$global.isActiveMatching"
        @click="useTopDisplay"
        class="top-show-button basic-button-design shadow-sm rounded"
      >
        매칭 상위 노출
        <br />
        <span
          v-if="isBlockUseTopMatchingDisplay"
          style="font-size: 16px; font-weight: normal; color: red"
        >
      ...
      <button
        @click="movePage('자기소개')"
        class="self-introduce-button basic-button-design shadow-sm rounded"
      ...
      <button
        @click="movePage('인연찾기')"
        class="matching-button basic-button-design shadow-sm rounded"
        :disabled="!$global.isLogin"
      >
        인연 찾기
      </button>
      ...
      <button
        @click="movePage('친구목록')"
        class="basic-button-design shadow-sm rounded"
        :disabled="!$global.isLogin"
      >
        친구 목록
      </button>
      ...
      <button
        v-if="$global.isAdmin && $global.isLogin"
        @click="movePage('신고관리')"
        class="basic-button-design shadow-sm rounded"
      >
        신고관리
...
export default {
  ...
  methods: {
      ...
      this.$http
        ...
        .catch((error) => {
          if (error.response.data.matchingTopDisplayUseingTime !== undefined) {
            this.$cookies.set(
              "matchingTopDisplayUseingTime",
              error.response.data.matchingTopDisplayUseingTime
            );
          }
    ...
    displayReUseWaitingTime: function () {
      let matchingTopDisplayUseingTime = Number(
        this.$cookies.get("matchingTopDisplayUseingTime")
      );
